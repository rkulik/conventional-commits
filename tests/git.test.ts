import { describe, expect, it } from '@jest/globals';
import { createCommitMessage, isCommit } from '../src/services/git';

describe('git', () => {
  it.each([
    {
      commit: { type: 'feat', scope: 'some-scope', description: 'some description' },
      expected: 'feat(some-scope): some description',
    },
    {
      commit: { type: 'feat', scope: '', description: 'some description' },
      expected: 'feat: some description',
    },
  ])('creates a commit message: "$expected"', testCase => {
    expect(createCommitMessage(testCase.commit)).toBe(testCase.expected);
  });

  it.each([
    { commit: { type: 'feat', scope: 'some-scope', description: 'some-description' }, expected: true },
    { commit: { type: 'feat', scope: '', description: 'some-description' }, expected: true },
    { commit: { type: 'feat', scope: '' }, expected: false },
    { commit: 'not a commit', expected: false },
    { commit: 1, expected: false },
  ])('validates a commit: $commit', testCase => {
    expect(isCommit(testCase.commit)).toBe(testCase.expected);
  });
});
